...
import com.example.nearby.data.model.mock.categories
...
fun NearbyCategoryFilterChipList(
    ...
    LaunchedEffect(key1 = selectedCategoryId) {
        val selectedCategotyOrNull = categories.find { it.id == selectedCategoryId }
        selectedCategotyOrNull?.let {
    ...
    LazyRow(
        ...
        items(items = categories, key = { it.id }) { category ->
            NearbyCategoryFilterChip(
                ...
                onClick = { isSelect ->
                    if (isSelect) selectedCategoryId = category.id
                }
            )
        }
    }
